...
func (query *ranking) LoadRankingAll() (*dto.RankingAll, error) {
	...
	if err := query.DB.Table("daily_works").Order("point desc").
		Select("users.name, users.screen_name, users.profile_image, daily_works.point, daily_works.ranking, daily_works.last_ranking").
		Joins("INNER JOIN users ON daily_works.screen_name = users.screen_name").
	...
	if err := query.DB.Table("daily_results").Order("point desc").
		Select("users.name, users.screen_name, users.profile_image, daily_results.point, daily_results.ranking, daily_results.last_ranking").
		Joins("INNER JOIN users ON daily_results.screen_name = users.screen_name").
	...
	if err := query.DB.Table("weekly_works").Order("point desc").
		Select("users.name, users.screen_name, users.profile_image, weekly_works.point, weekly_works.ranking, weekly_works.last_ranking").
		Joins("INNER JOIN users ON weekly_works.screen_name = users.screen_name").
	...
	if err := query.DB.Table("weekly_results").Order("point desc").
		Select("users.name, users.screen_name, users.profile_image, weekly_results.point, weekly_results.ranking, weekly_results.last_ranking").
		Joins("INNER JOIN users ON weekly_results.screen_name = users.screen_name").
	...
	if err := query.DB.Table("monthly_works").Order("point desc").
		Select("users.name, users.screen_name, users.profile_image, monthly_works.point, monthly_works.ranking, monthly_works.last_ranking").
		Joins("INNER JOIN users ON monthly_works.screen_name = users.screen_name").
	...
	if err := query.DB.Table("monthly_results").Order("point desc").
		Select("users.name, users.screen_name, users.profile_image, monthly_results.point, monthly_results.ranking, monthly_results.last_ranking").
		Joins("INNER JOIN users ON monthly_results.screen_name = users.screen_name").
